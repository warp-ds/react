...
const Page = React.forwardRef<React.AnchorHTMLAttributes<HTMLAnchorElement>, PageProps>(({ page = 1, className, currentPage, noFollow, ...props }, ref) => {
  ...
  return (
    <a aria-label={ariaLabel} {...props} ref={ref as Ref<HTMLAnchorElement>} rel={`${noFollow ? 'nofollow' : ''}`} aria-current={isCurrentPage ? 'page' : undefined} className={classNames(className, ccPagination.link, [isCurrentPage ? ccPagination.active : ccPagination.notActive])}>
      {page}
    </a>
  );
});
